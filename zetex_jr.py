...
def check_owner(ctx):
    return ctx.author.id in [797942648932794398, 190804082032640000, 302920327699103744, 900479607202017330, 227916041567731722, 349234259099254794]
...
async def manual(ctx,
    ...
    if ctx.author.id in [797942648932794398, 190804082032640000, 302920327699103744, 900479607202017330, 150040671443484673, 1030552414924836976, 248279472917315594, 227916041567731722, 349234259099254794]:
...
async def epinephrine(ctx):
    ...
    if randomRoll == 999999999:
        ...
        channel = tracker_bot.get_channel(1229914924994662420)
        await channel.send("NEW HIGHEST: " + randomRoll + "<@&1229924994986016778>")
        ...
        randomRoll = format_num(randomRoll)
        channel = tracker_bot.get_channel(1229914924994662420)
        await channel.send("NEW LOWEST: " + randomRoll + "<@&1229924994986016778>")
        await ctx.respond(f"you didn't get epinephrine :( \n(got {randomRoll} but needed 999,999,999)\nonly {distance} away!\n# NEW LOWEST ROLL!")
    else:
        randomRoll = format_num(randomRoll)
        await ctx.respond(f"you didn't get epinephrine :( \n(got {randomRoll} but needed 999,999,999)")
