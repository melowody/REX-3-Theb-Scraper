...
async def send_lb_message(ctx: commands.Context, title: str, tracks: list[RExTrack],
                          formatter: Callable[[RExTrack], str]) -> None:
    messages: list[str] = []
    out = f"## {title}"
...
class RExDiscordLeaderboardCommand(commands.Cog):
    ...
    @commands.hybrid_group(name="leaderboard", aliases=["lb"],
                           description="Look at the rarest finds of people, guilds, or across the bot!")  # type: ignore[arg-type]
    async def leaderboard(self, ctx: commands.Context):
        await ctx.reply(":D")

    @leaderboard.command(name="guild",
                         description="Get the rarest finds in the current Guild")  # type: ignore[arg-type]
    async def guild_lb(self, ctx: commands.Context, adjusted: typing.Optional[bool] = False,
                       limit: typing.Optional[int] = 10, start: typing.Optional[int] = 0):
        guild = ctx.guild
        if guild is None:
        ...
        await send_lb_message(
            ...
            rarests,
            lambda
                x: f"{x.player_name} - {"" if not isinstance(var := x.get_variant(), RExVariant) else (var.variant_name + " ")}{"NotInIndex" if not isinstance(ore := x.get_ore(), RExOre) else ore.ore_name} (1 in {get_rarity(x, adjusted):,})")
    ...
    @app_commands.autocomplete(
        player=get_items(RExPlayerManager().get_all(), lambda x: str(x.user_id), lambda x: x.player_name,
                         predicate=lambda interaction, player: check_mutual_guilds(interaction.user.id, player))
    )
    async def player_lb(self, ctx: commands.Context, player: str, adjusted: typing.Optional[bool] = False,
                        limit: typing.Optional[int] = 10, start: typing.Optional[int] = 0):
        ...
        await send_lb_message(
            ...
            rarests,
            lambda
                x: f"{"" if not isinstance(var := x.get_variant(), RExVariant) else (var.variant_name + " ")}{"NotInIndex" if not isinstance(ore := x.get_ore(), RExOre) else ore.ore_name} (1 in {get_rarity(x, adjusted):,})"
        )

    @leaderboard.command(name="global", description="Get the rarest finds from all players registered to the bot")
    async def global_lb(self, ctx: commands.Context, adjusted: typing.Optional[bool] = False,
                        limit: typing.Optional[int] = 10):
        rarests = track_query(None)
        await send_lb_message(
            ctx,
            f"Global Top {min(limit, len(rarests))}",
            list(filter(lambda x: isinstance(RExPlayerManager().get_one(lambda j: j.player_name == x.player_name, None),
                                             RExPlayer), rarests)),
            lambda
                x: f"{x.player_name} - {"" if not isinstance(var := x.get_variant(), RExVariant) else (var.variant_name + " ")}{"NotInIndex" if not isinstance(ore := x.get_ore(), RExOre) else ore.ore_name} (1 in {get_rarity(x, adjusted):,})"
        )
